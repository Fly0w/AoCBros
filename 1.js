...
// console.log("First part result" + sum);

// PART TWO

// const liste = `two1nine
// eightwothree
// abcone2threexyz
// xtwone3four
// 4nineeightseven2
...
// For each element, check if each entry of dico is inside.

function lastDigit(item) {
  let lastStr = "";
  for (let i = item.length - 1; i >= 0; i--) {
    // console.log(i + "   " + lastStr);
    if (numbers.includes(item[i])) {
      return Number(item[i]);
    } else {
      lastStr = item[i] + lastStr;

      for (let entry in dico) {
        if (lastStr.includes(dico[entry])) {
          return Number(entry);
        }
      }
    }
...
array.forEach((item) => {
  let first = firstDigit(item);
  let last = lastDigit(item);
